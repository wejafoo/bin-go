...
func main() {
	...
	if Fd.FdInit != "" {
		...
		case "ng", "angular":
			if success := blib.NewAngular(); !success {
				log.Fatalf("\n%s", blib.GetAngularError())
			}
		case "ts", "typescript":
			if success := blib.NewTypescript(); !success {
				log.Fatalf("\n%s", blib.GetTypescriptError())
			}
		case "njs", "nodejs":
			if success := blib.NewNodejs(); !success {
				log.Fatalf("\n%s", blib.GetNodejsError())
			}
		case "go":
			if success := blib.NewGo(); !success {
				log.Fatalf("\n%s", blib.GetGoError())
			}
		case "py", "python":
			if success := blib.NewPython(); !success {
				log.Fatalf("\n%s", blib.GetPythonError())
			}
		case "do", "docker":
			if success := blib.NewDocker(); !success {
...
func getCleanConfig(Fdc blib.FDC) blib.FDC {
	Fda = blib.FDA{
		...
		AdcPtr:              flag.String("adc", Fdc.FdAdc, "REQUIRED - String indicating the location of default remote credentials"),
		BuildContextPtr:     flag.String("context", Fdc.FdBuildContext, "REQUIRED - String indicating local(-local) or cloud(-remote) deploy"),
